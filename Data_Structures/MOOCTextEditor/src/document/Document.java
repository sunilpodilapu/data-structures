...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		Matcher m = syllablePattern.matcher(word);
		int count = 0;
		...
		for(String syllable : syllables) {
			count++;
			if(count == syllables.size() && syllable.equals("e") &&
	...
	public double getFleschScore()
	{
		...
		double numSent = (double) getNumSentences();
		double numSyll = (double) getNumSyllables();

		double fleschScore = 206.835 - 1.015 * (numWords / numSent) -
				84.6 * (numSyll / numWords);
